...
interface USSwingCandidate {
  ...
  matched_conditions: SwingCondition[];
}

interface USSwingOverview {
  research: {
    ready: boolean;
  ...
  agent: {
    ...
    configured_us_symbols: number;
    expected_us_symbols: number;
    benchmark_symbol: string;
    strategy_enabled: boolean;
    active_strategies: string[];
  };
  candidates: USSwingCandidate[];
}
...
function regimeClasses(regime: USSwingCandidate['market_regime']) {
...
export default function USSwingRadarPage() {
  const overviewQuery = useQuery<USSwingOverview>({
    queryKey: ['us-swing-radar-overview'],
    queryFn: () => apiFetch('/us-swing-radar/overview?limit=60&min_score=60'),
    ...
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <div className="flex items-center gap-3 rounded-2xl border border-slate-800 bg-slate-900/70 px-5 py-4 text-sm text-slate-300">
          ...
          Loading US swing research snapshot...
        </div>
      </div>
    );
  }
  ...
  if (overviewQuery.isError || !overviewQuery.data) {
    return (
      <section className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-6">
        <p className="text-sm font-medium text-rose-200">US Swing Radar failed to load.</p>
  ...
  const { research, agent, candidates } = overviewQuery.data;
  ...
  const bestFilters = research.tuning?.best_filters;

  return (
    <div className="space-y-6">
      <section className="rounded-[28px] border border-slate-800 bg-[radial-gradient(circle_at_top_left,_rgba(14,165,233,0.18),_transparent_28%),linear-gradient(135deg,rgba(15,23,42,0.96),rgba(2,6,23,0.99))] p-6 shadow-[0_18px_70px_rgba(2,6,23,0.45)]">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-sky-500/20 bg-sky-500/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.24em] text-sky-200">
              ...
              Sector-Balanced US 200
            </div>
            <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-100">
              US Swing Radar
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
              Daily swing board for the 200-name US sector universe. It ranks names from the
              10-year research run, exposes the tuned live filters, and tracks whether the agent
              is currently trading that strategy.
            </p>
          </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          />
          <SummaryCard
            title="Agent US Universe"
            value={`${agent.configured_us_symbols}/${agent.expected_us_symbols}`}
            subtitle={`${agent.active_strategies.length} active strategies · benchmark ${agent.benchmark_symbol}`}
            icon={Target}
          />
          <SummaryCard
            title="Tuned Filter"
            value={bestFilters ? `${bestFilters.min_score}` : '—'}
            subtitle={bestFilters ? `prob ${bestFilters.min_probability} · edge ${bestFilters.min_edge}` : 'Tuning summary unavailable'}
            icon={Radar}
          />
        </div>
      </section>
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
            ...
            <div className="mt-4 space-y-3 text-sm leading-6 text-slate-300">
              <p>
                ...
                These ranks come from that completed artifact snapshot and use <span className="font-medium text-slate-100">{agent.benchmark_symbol}</span> as the benchmark regime anchor.
              </p>
              <p>
                `US_Swing_Radar` is {agent.strategy_enabled ? 'enabled' : 'disabled'} and the configured agent universe currently
                covers <span className="font-medium text-slate-100">{agent.configured_us_symbols}/{agent.expected_us_symbols}</span> of the curated US names.
              </p>
              <p>
                Best tuned live filter is currently score <span className="font-medium text-slate-100">{bestFilters?.min_score ?? '—'}</span>,
                probability <span className="font-medium text-slate-100">{bestFilters?.min_probability ?? '—'}</span>,
                edge <span className="font-medium text-slate-100">{bestFilters?.min_edge ?? '—'}</span>.
              </p>
            </div>
