"""Background publisher for execution events and broker updates."""
...
class ExecutionEventPublisher:
    """Fan out normalized execution events to transport and analytics backends."""
    ...
    async def stop(self) -> None:
        ...
        for task in (self._agent_task, self._broker_task, self._publish_task):
    ...
    async def _connect_backends(self) -> None:
        ...
        if self._settings.clickhouse_enabled:
            try:
                ...
                await self._ensure_clickhouse_table()
            except Exception as exc:
    ...
    async def _publish_envelope(self, envelope: dict[str, Any]) -> None:
        ...
        if self._nats_client is not None:
            subject = f"{self._settings.nats_stream_prefix}.execution.events"
            tasks.append(asyncio.create_task(self._nats_client.publish(subject, self._dumps_bytes(envelope))))
        if self._kafka_producer is not None:
            topic = f"{self._settings.kafka_topic_prefix}.execution.events"
            tasks.append(asyncio.create_task(self._kafka_producer.send_and_wait(topic, envelope)))
        if self._settings.clickhouse_enabled:
    ...
    async def _ensure_clickhouse_table(self) -> None:
    ...
    async def _write_clickhouse(self, envelope: dict[str, Any]) -> None:
        ...
        response = await self._http_client.post(
            ...
            params={
                "query": (
                    f"INSERT INTO {self._settings.clickhouse_database}.execution_events FORMAT JSONEachRow"
                )
            },
